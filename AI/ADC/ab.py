...
from model.netw1 import TGADC,TGADC_ab
...
def train(model, device, train_loader, optimizer, criterion):
    ...
    for batch_idx, data in enumerate(train_loader):
        heavy, light, antigen, playload, linker, dar, label ,playload_graph,linker_graph,adc_graph= data[:]
        ...
        playload = torch.FloatTensor(np.array(playload)).to(device)
        linker = torch.FloatTensor(np.array(linker)).to(device)
        ...
        output = model(heavy=heavy, light=light, antigen=antigen,
                       playload_graph=playload_graph, linker_graph=linker_graph, dar=dar,adc=adc_graph,playload=playload,linker=linker)
        # return
...
if __name__ == '__main__':
    ...
    dar_data = df['dar'].values.reshape(-1, 1)
    ...
    for train_index, val_index in kf.split(df):
        ...
        # 分离训练集和验证集
            ...
            # train_fold, val_fold = train_test_split(train_fold, test_size=30)
            train_fold, val_fold = train_test_split(train_fold, test_size=0.09, random_state=SEED, stratify=train_fold['label'])

            test_fold = df.iloc[val_index].reset_index(drop=True)
            ...
            train_set = ADCDatasetmorgan(train_fold_balanced)
            test_set = ADCDatasetmorgan(test_fold)
            val_set=ADCDatasetmorgan(val_fold)
            ...
            model = TGADC_ab(compound_dim=128, protein_dim=128, gt_layers=3, gt_heads=4, out_dim=1)
            model.to(device)
            ...
            criterion = BCEFocalLoss(alpha=0.3)
            ...
            for epoch in range(epochs):
                ...
                if vmcc > best_mcc:
                    ...
                    print('fold ',i_fold,' mcc improved at epoch ', best_epoch, ';\tbest_mcc:', best_mcc)
                else:
                    patience += 1
                # epoch += 1
