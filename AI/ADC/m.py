...
from model.netw1 import UNADC_model
...
if __name__ == '__main__':
    ...
    train_fold=df
    # train_fold, val_fold = train_test_split(train_fold, test_size=30,random_state=10)
    ...
    # pca_processor.save_model('scaler.joblib', 'pca.joblib')
    
    # pca_processor.load_model('path_to_save_scaler.joblib', 'path_to_save_pca.joblib')

    # # 现在可以使用 transform 方法处理新数据了
    ...
    # train_set = ADCDatasetmorgan(train_fold_balanced)#没有pca
    ...
    model = UNADC_model(device=device,compound_dim=128, protein_dim=128, gt_layers=3, gt_heads=4, out_dim=1)
    # model = net()
    ...
    for epoch in range(epochs):
        ...
        print(train_loss)
        torch.save({'model': model.state_dict()}, 'UNADC_model.pth')
